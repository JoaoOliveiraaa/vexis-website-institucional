...
import { ChevronRight, Plus, Target, UserCheck } from "lucide-react"
...
export default async function MyTasksPage() {
  ...
  const tasks = (tasksData || []).filter((task) => {
    const isPrimaryAssignee = task.assigned_to === user.id
    const isCreator = task.created_by === user.id
    const isCollaborator =
      allAssignees?.some((assignee) => assignee.task_id === task.id && assignee.user?.id === user.id) ?? false

    return isPrimaryAssignee || isCreator || isCollaborator
  })
  ...
  const quickStats = [
    {
      key: "backlog",
      label: "Backlog",
      description: "Ideias que você registrou para priorização.",
      accent: brandGradient,
    },
    {
      key: "todo",
      label: "A Fazer",
      description: "Itens aguardando seu start.",
      accent: brandGradient,
    },
    {
      key: "in_progress",
      label: "Em Progresso",
      description: "Demandas em execução sob sua responsabilidade.",
      accent: brandGradient,
    },
    {
      key: "review",
      label: "Revisão",
      description: "Entregas aguardando validação sua ou do time.",
      accent: brandGradient,
    },
    {
      key: "done",
      label: "Concluído",
      description: "Resultados finalizados por você ou seu squad.",
      accent: brandGradient,
    },
  ]

  const myTaskIds = new Set(tasks.map((task) => task.id))
  const taskAssignees = (allAssignees || []).filter((assignee) => myTaskIds.has(assignee.task_id))
    ...
    <div className="space-y-8">
        ...
        <div className="relative flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-3">
            ...
            <p className="max-w-2xl text-sm text-white/80 lg:text-base">
              Acompanhe entregas onde você é responsável, colabora ou criou. Planeje sua semana, elimine gargalos e
              mantenha o time informado com transparência.
            </p>
            <div className="flex flex-wrap gap-3 text-sm text-white/80">
              <div className="flex items-center gap-2 rounded-full bg-white/10 px-4 py-2">
                <UserCheck className="h-4 w-4" />
                {tasks.length} tarefas sob sua responsabilidade
              </div>
              <div className="flex items-center gap-2 rounded-full bg-white/10 px-4 py-2">
                <Target className="h-4 w-4" />
                Organize prioridades para a sprint atual
              </div>
            </div>
          </div>
          <div className="flex flex-col items-start gap-3 rounded-2xl border border-white/20 bg-white/10 p-6 text-sm text-white/80 backdrop-blur-lg lg:max-w-sm">
            <p className="text-base font-semibold text-white">Próximo passo sugerido</p>
            <p>
              Valide as tarefas em revisão e negocie prazos de itens críticos que estão sem responsável adicional.
            </p>
            <Link
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Visualize apenas as entregas onde você atua diretamente e organize o foco do dia.
          </p>
        </div>
            ...
            <CardContent className="flex flex-col gap-3 px-5 py-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-2xl font-semibold text-card-foreground">{tasksByStatus[stat.key] || 0}</p>
                </div>
                <div
      ...
      <Card className="border border-border/60 bg-card/95 shadow-sm">
        <CardContent className="p-0">
          <TasksTable tasks={tasks} taskAssignees={taskAssignees} />
        </CardContent>
      </Card>
    </div>
  )
